...
public class IndexedPagedList<E> extends AbstractSequentialList<E> implements List<E> {
  ...
  public IndexedPagedList() {
	...
	head = new Page<>(null, null);
	tail = new Page<>(head, null);
  ...
  public boolean add(E item) {
	...
	if(item == null) throw new NullPointerException("The item being appended cannot be null");
  ...
  public void add(int pos, E item) {
    // TODO
	  ...
	  if(pos < 0 || pos > size()) throw new IndexOutOfBoundsException("Item cannot be out of the range");
	  
	  modificationCount++;
	  totalSize++;
	  
	  if(size() == 1) {
		  Page<E> newPageInsert = new Page<>(head, tail);
		  
		  newPageInsert.items[0] = item;
		  newPageInsert.count++;
		  
		  head.next = newPageInsert;
		  tail.prev = newPageInsert;
		  
		  pageIndex.add(new IndexEntry<>(newPageInsert, 0, 1));
		  return;
		  }
	  
	  PageInfo<E> pageInfo = findPageForLogicalIndex(pos); 
	  
	  if(pageInfo.page.count < PAGE_CAPACITY) { //how many items are stored in the page
		  pageInfo.page.addItem(pageInfo.offset, item);
		  
		  int pageIndx = findIndexInPageIndex(pageInfo.page);
		  pageIndex.get(pageIndx).count = pageInfo.page.count;
		  
		  updatePageIndex(pageIndx+ 1, 1);
		  return;
	  }
	  
		  Page<E> newPage = new Page<>(pageInfo.page, pageInfo.page.next);
		  
		  pageInfo.page.next.prev = newPage;
		  pageInfo.page.next = newPage;
		 
		  newPage.items[0] = pageInfo.page.items[2];
		  newPage.items[1] = pageInfo.page.items[3];
		  newPage.count = 2;
		  
		  pageInfo.page.items[2] = null;
		  pageInfo.page.items[3] = null;
		  pageInfo.page.count = 2;
		  
		  if(pageInfo.offset <= 2) {
			  pageInfo.page.addItem(pageInfo.offset, item);
		  } else {
			  newPage.addItem(pageInfo.offset - 2, item);
		  }
		  
		  
		  pageIndex.add(findIndexInPageIndex(pageInfo.page) + 1, new IndexEntry<>(newPage, pos - pageInfo.offset + pageInfo.page.count, newPage.count));
		  
		  updatePageIndex(findIndexInPageIndex(pageInfo.page) + 2, 1);
  }
  ...
  public E remove(int pos) {
    // TODO
	  if(pos < 0 || pos >= size()) throw new IndexOutOfBoundsException("Pos is out of range");
	  
	  PageInfo<E> pageInfo = findPageForLogicalIndex(pos);
	  
	  int OGPageIndex = findIndexInPageIndex(pageInfo.page);
	  modificationCount++;
	  totalSize--;
	  
	  E returnValue = pageInfo.page.removeItem(pageInfo.offset);
	  
	  int pageIndx = findIndexInPageIndex(pageInfo.page);
	  if(pageIndx != -1) {
		  IndexEntry<E> currEntry = pageIndex.get(pageIndx);
		  currEntry.count = pageInfo.page.count;
	  }
	  
	  if(size() == 0) {
		  Page<E> prev = pageInfo.page.prev;
		  Page<E> next = pageInfo.page.next;
	  ...
	  if(pageInfo.page.count < HALF_CAPACITY) {
		  Page<E> succ = pageInfo.page.next;
	  
	  
		  if(succ != tail) {
			  int succIndx = findIndexInPageIndex(succ);
			  IndexEntry<E> succEntry = pageIndex.get(succIndx);
			  IndexEntry<E> currEntry = pageIndex.get(findIndexInPageIndex(pageInfo.page));
			  
			  if(succ.count > HALF_CAPACITY) {
				  pageInfo.page.addItem(pageInfo.page.count, succ.removeItem(0));
				  
				  currEntry.count = pageInfo.page.count;
				  succEntry.count = succ.count;
				  
				  succEntry.logicalIndex += 1;
;			  }
			  
			  else {
				  int moveCount = succ.count;
				  ...
				  Page<E> afterSucc = succ.next;
				  pageInfo.page.next = afterSucc;
				  afterSucc.prev = pageInfo.page;
				  
				  pageIndex.remove(succIndx);
			  }
		  }
	  }
		  	else {
		  		if(pageInfo.page.count == 0) {
		  			Page<E> prev = pageInfo.page.prev;
		  			Page<E> next = pageInfo.page.next;
		  			...
		  			if(findIndexInPageIndex(pageInfo.page) != -1) pageIndex.remove(findIndexInPageIndex(pageInfo.page));
		  		}
		  	}
		  

	  int index = findIndexInPageIndex(pageInfo.page);
	  
	  if(index == -1) index = OGPageIndex;
  ...
  public ListIterator<E> listIterator(int pos) {
    // TODO
	  if(pos < 0 || pos > size()) throw new IndexOutOfBoundsException("Index is out of range");
  ...
  private PageInfo<E> findPageForLogicalIndex(int pos) {
    // TODO
	  if(size() == 0) return new PageInfo<>(tail, 0);
	  
	  if(pos == size()) {
		  IndexEntry<E> lastEntry = pageIndex.get(pageIndex.size() - 1);
		  Page<E> lastPage = lastEntry.page;
		  return new PageInfo<>(lastPage, lastPage.count);
	  }
	  int index = Collections.binarySearch(pageIndex, pos, INDEX_COMPARATOR);
	  
	  if(index >= 0) { //binary searching is searching by the first index
		  return new PageInfo<E>(pageIndex.get(index).page, 0);
	  }
	  
	  //index < 0 means pos was not found in the first index of the page
	  int insertionPoint = -index - 1;
	  int pageIndx = insertionPoint - 1;
	  
	  IndexEntry<E> entry = pageIndex.get(pageIndx);
	  
	  int offset = pos - entry.logicalIndex; //offset is where we want to actually insert
	  
	  return new PageInfo<E>(entry.page, offset);
  }
  ...
  private class PagedListIterator implements ListIterator<E> {
    ...
    public E next() {
      // TODO
    	...
    	E value = currentPage.items[pageOffset];
    	...
    	if(pageOffset >= currentPage.count && currentPage.next != null) {
    		currentPage = currentPage.next;
    		pageOffset = 0;
    	}
    ...
    public E previous() {
      // TODO
    	...
    	return currentPage.items[pageOffset];
    }
    ...
    public void remove() {
      // TODO
    	...
    	if(lastItemIndex < 0) throw new IllegalStateException("need to call next() or previous(), or remove()/add() have been called since last move");
    	
    	IndexedPagedList.this.remove(lastItemIndex);
    	
    	if(lastDirection == Direction.NEXT) logicalIndex--;
    	
    	PageInfo<E> pageInfo = findPageForLogicalIndex(logicalIndex);
    	currentPage = pageInfo.page;
    	pageOffset = pageInfo.offset;
    ...
    public void set(E item) {
      // TODO
    	...
    	if(lastItemIndex < 0) throw new IllegalStateException("Next() or previous() need to be called or remove()/add() have been called since last move");
    	
    	PageInfo<E> pageInfo = findPageForLogicalIndex(lastItemIndex);
    	
    	pageInfo.page.items[pageInfo.offset] = item; 
    }
